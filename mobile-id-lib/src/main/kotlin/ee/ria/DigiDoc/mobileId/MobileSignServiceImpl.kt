...
@Singleton
class MobileSignServiceImpl : MobileSignService {
        ...
        if (request != null) {
                ...
                if (certificateCertBundle != null) {
                    midRestServiceClient =
                        ServiceGenerator.createService(
                            MIDRestServiceClient::class.java,
            ...
            try {
                ...
                if (!responseWrapper.isSuccessful) {
                    ...
                    if (isResponseError(responseWrapper, response, MobileCreateSignatureCertificateResponse::class.java)
                    ) {
                        return
                    }
                ...
                errorLog(
                    ...
                    "Failed to sign with Mobile-ID. REST API certificate request failed. " +
                        "Unknown host. Exception message: ${e.message}. Exception: ${e.stackTrace.contentToString()}",
                    e,
                )
    ...
    private suspend fun doCreateSignatureStatusRequestLoop(request: GetMobileCreateSignatureSessionStatusRequest) {
        ...
        if (response != null && response.state?.let { isSessionStatusRequestComplete(it) } == true) {
            if (isResponseError(responseWrapper, response, MobileCreateSignatureSessionStatusResponse::class.java)) {
            ...
            delay(SUBSEQUENT_STATUS_REQUEST_DELAY_IN_MILLISECONDS)
