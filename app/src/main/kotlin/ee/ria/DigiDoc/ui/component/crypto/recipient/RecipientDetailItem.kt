...
data class RecipientDetailItem(
    ...
    fun recipientDetailItems(
        ...
        if (recipient.certType == CertType.PasswordType) {
            passwordRecipientItems(recipient)
        } else {
            certRecipientItems(recipient, recipientFormattedName, recipientIssuerName, recipientConcatKDFAlgorithmURI)
        }

    @Composable
    private fun passwordRecipientItems(recipient: Addressee): List<RecipientDetailItem> =
        listOf(
            item(
                label = R.string.recipient_details_name_label,
                value = recipient.lockLabel,
                testTag = "recipientLockLabel",
            ),
            item(
                label = R.string.recipient_details_lock_type_label,
                value = recipient.lockType,
                testTag = "recipientLockType",
            ),
        )

    @Composable
    private fun certRecipientItems(
        recipient: Addressee,
        recipientFormattedName: String?,
        recipientIssuerName: String?,
        recipientConcatKDFAlgorithmURI: String?,
        ...
        val validToFormatted = recipient.validTo?.let { dateFormat.format(it) }
        return listOf(
            item(
                icon =
