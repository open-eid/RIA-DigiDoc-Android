...
fun SignatureInputScreen(
    ...
    var mobileIdPhone by remember { mutableStateOf("") }
    var mobileIdPersonalCode by remember { mutableStateOf("") }

    var smartIdCountry by remember { mutableIntStateOf(0) }
    var smartIdPersonalCode by remember { mutableStateOf("") }

    var nfcCanNumber by remember { mutableStateOf("") }
    ...
    Scaffold(
        ...
        Column(
            ...
            when (chosenMethod) {
                SigningMethod.MOBILE_ID ->
                    MobileIdView(
                        ...
                        fields = { phoneNo, personalCode ->
                            mobileIdPhone = phoneNo
                            mobileIdPersonalCode = personalCode
                        },
                        signAction = { action ->
                            signAction = action
                        },
                ...
                SigningMethod.SMART_ID ->
                    SmartIdView(
                        ...
                        fields = { country, personalCode ->
                            smartIdCountry = country
                            smartIdPersonalCode = personalCode
                        },
                        signAction = { action ->
                            signAction = action
                        },
            ...
            if (!isIdCardSigning && (chosenMethod != SigningMethod.NFC || nfcSupported)) {
                ...
                Button(
                    onClick = {
                        val dataStore = sharedSettingsViewModel.dataStore

                        when (chosenMethod) {
                            SigningMethod.MOBILE_ID -> {
                                dataStore.setPhoneNo(if (rememberMe) mobileIdPhone else "")
                                dataStore.setPersonalCode(if (rememberMe) mobileIdPersonalCode else "")
                            }
                            SigningMethod.SMART_ID -> {
                                dataStore.setCountry(if (rememberMe) smartIdCountry else 0)
                                dataStore.setSidPersonalCode(if (rememberMe) smartIdPersonalCode else "")
                            }
                            SigningMethod.NFC -> {
                                dataStore.setCanNumber(if (rememberMe) nfcCanNumber else "")
                            }
